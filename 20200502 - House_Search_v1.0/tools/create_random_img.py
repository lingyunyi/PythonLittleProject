...
def generate_captcha(width=140, height=60, length=4):
    ...
    font = ImageFont.truetype("../static/Public/ttf/abc.ttf", size=36)
    # 验证码文本
    text = ""
    for i in range(length):
        ...
        draw.text((width * 0.2 * (i + 1) + rand_len, height * 0.2 + rand_len), c, fill=get_random_color(),font=font)
    # 加入干扰线
    for i in range(3):
        x1 = randint(0, width)
        y1 = randint(0, height)
    ...
    # 保存图片
    img.save("../static/Public/img/temp/" + text + ".jpg")
    return text + ".jpg"
